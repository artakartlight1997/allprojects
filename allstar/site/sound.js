...
function sfxCall() {
  if (!A.ctx) return;
  bleep(anow(), [79, 84], 0.05, 0.09, 0.13);
}
// 何かに あてた／とった
function sfxHit() {
  if (!A.ctx) return;
  const t = anow();
  bleep(t, [84, 91], 0.03, 0.05, 0.11);
  nz(t, 0.05, 0.08, 2000, 7000);
}
// ふわっと とぶ
function sfxFlap() {
  if (!A.ctx) return;
  tone(anow(), 72, 0.07, 0.09, 'square', null, 84);
}
// ミニゲーム せいこう
function sfxWin() {
  if (!A.ctx) return;
  bleep(anow(), [76, 83, 88], 0.045, 0.08, 0.14);
}
// ミニゲーム しっぱい（1人 ぬける）
function sfxMiss() {
  if (!A.ctx) return;
  const t = anow();
  bleep(t, [64, 59, 54], 0.08, 0.14, 0.14);
  nz(t, 0.14, 0.10, 200, 1200);
}
// スピードアップ
function sfxSpeed() {
  if (!A.ctx) return;
  bleep(anow(), [72, 76, 79, 84, 88], 0.05, 0.10, 0.14);
}
// ボス とうじょう
function sfxBoss() {
  if (!A.ctx) return;
  const t = anow();
  bleep(t, [48, 48, 51, 48, 55, 54], 0.13, 0.22, 0.15);
  kick(t, 0.9); kick(t + 0.26, 0.9); kick(t + 0.52, 0.9);
}
// ボス げきは
function sfxBossDown() {
  if (!A.ctx) return;
  const t = anow();
  bleep(t, [84, 88, 91, 96, 91, 96, 100], 0.07, 0.13, 0.16);
  kick(t, 0.9); kick(t + 0.35, 0.9);
  nz(t + 0.1, 0.5, 0.12, 100, 1500);
}
