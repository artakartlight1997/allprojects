...
function sfxBounce(hard) {
  if (!A.ctx) return;
  const t = anow();
  tone(t, hard ? 78 : 72, 0.07, 0.14, 'square');
  nz(t, 0.04, 0.06, 1800, 6000);
}

// ブロックを こわした（つづけて あたると 音が 上がる）
function sfxBrick(n) {
  if (!A.ctx) return;
  const t = anow();
  const m = 72 + Math.min(14, n || 0);
  tone(t, m, 0.10, 0.15, 'triangle');
  tone(t + 0.01, m + 12, 0.07, 0.08, 'square');
  nz(t, 0.05, 0.07, 2400, 8000);
}
...
function sfxHard() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 55, 0.09, 0.14, 'square');
  nz(t, 0.06, 0.10, 300, 2200);
}
...
function sfxBomb() {
  if (!A.ctx) return;
  const t = anow();
  nz(t, 0.36, 0.30, 60, 3000);
  tone(t, 42, 0.30, 0.22, 'sine', null, 26);
}
...
function sfxLaser() {
  if (!A.ctx) return;
  tone(anow(), 90, 0.09, 0.10, 'sawtooth', null, 70);
}
...
function sfxLose() {
  if (!A.ctx) return;
  const t = anow();
  [64, 60, 55, 50].forEach((m, i) => tone(t + i * 0.11, m, 0.24, 0.18, 'sawtooth'));
}
...
function sfxPapaDown() {
  if (!A.ctx) return;
  const t = anow();
  for (let i = 0; i < 5; i++) nz(t + i * 0.1, 0.22, 0.22 - i * 0.02, 60, 2800 - i * 300);
  [72, 76, 79, 84].forEach((m, i) => tone(t + 0.6 + i * 0.12, m, 0.3, 0.20, 'triangle'));
}
...
function sfxTest() {
  if (!A.ctx) return;
  sfxBounce(false);
  setTimeout(() => sfxBrick(1), 220);
  setTimeout(() => sfxBrick(5), 380);
  setTimeout(() => sfxItem(), 620);
  setTimeout(() => sfxBomb(), 1000);
}

// --- BGM ----------------------------------------------------------------------

const BGM = {
  prog: [[0, 5, 7, 5], [0, 3, 5, 7], [0, 7, 5, 3]],
  min: [[1], [0, 1, 3], [2]],
};

let bgmOn = false, bgmRoot = 57, bgmSet = 0, bgmBpm = 116, bgmHot = 0;
...
function bgmBar(t0, bar, spb) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (i === 1 || i === 3) nz(t0 + i * spb, 0.08, 0.15, 900, 4200, A.mus);
    nz(t0 + i * spb + spb / 2, 0.03, 0.05, 6000, 11000, A.mus);
  }
  for (let i = 0; i < 4; i++) {
    tone(t0 + i * spb, r - 24, spb * 0.7, 0.16, 'triangle', A.mus);
  }
  pad(t0, [r, r + third, r + 7], spb * 3.8, 0.12);
  const mel = [7, 12, third, 7];
  for (let i = 0; i < 4; i++) {
    tone(t0 + i * spb + spb * 0.5, r + mel[i], spb * 0.42, 0.10, 'square', A.mus);
  }
}
