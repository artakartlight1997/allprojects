...
const BGM = {
  prog: [[0, 3, 5, 7], [0, 7, 3, 5]],
  min: [[1], [2]],
};

let bgmOn = false, bgmRoot = 58, bgmSet = 0, bgmBpm = 126, bgmHot = 0;
...
function sfxLand() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 52, 0.08, 0.10, 'sine', A.sfx, 44);
  nz(t, 0.05, 0.08, 200, 1400);
}
function sfxClearBlob() {
  if (!A.ctx) return;
  const t = anow();
  [72, 79, 84].forEach((m, i) => tone(t + i * 0.05, m, 0.16, 0.12, 'triangle'));
  nz(t, 0.10, 0.09, 800, 5000);
}
// れんさの ぶんだけ 音が 高く なる（何れんさか 耳でも わかる）
function sfxChain(n) {
  if (!A.ctx) return;
  const t = anow();
  const base = 70 + Math.min(7, n) * 3;
  [0, 4, 7, 12].forEach((m, i) => tone(t + i * 0.055, base + m, 0.22, 0.14, 'triangle'));
  kick(t, 0.8);
  if (n >= 3) pad(t, [base - 24, base - 17, base - 12], 0.9, 0.22);
}
