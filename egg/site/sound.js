...
function bgmBar(t0, bar, spb) {
  ...
  for (let i = 0; i < 8; i++) {
    tone(t0 + i * (spb / 2), r - 24 + (i % 4 === 2 ? 7 : 0), spb * 0.44, 0.1, 'square', A.mus);
  }
...
function sfxEat() {
  if (!A.ctx) return;
  const t = anow();
  nz(t, 0.06, 0.12, 300, 1800);
  nz(t + 0.13, 0.06, 0.10, 300, 1800);
  tone(t + 0.26, 79, 0.12, 0.10, 'triangle', A.sfx, 84);
}
function sfxBath() {
  if (!A.ctx) return;
  const t = anow();
  nz(t, 0.5, 0.09, 900, 6000);
  for (let i = 0; i < 5; i++) tone(t + i * 0.07, 76 + i * 3, 0.10, 0.06, 'sine');
}
function sfxSleep() {
  if (!A.ctx) return;
  const t = anow();
  [67, 63, 60].forEach((m, i) => tone(t + i * 0.16, m, 0.42, 0.11, 'sine'));
}
function sfxPoop() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 46, 0.16, 0.08, 'sine', A.sfx, 38);
  nz(t, 0.10, 0.06, 150, 700);
}
function sfxHatch() {
  if (!A.ctx) return;
  const t = anow();
  nz(t, 0.10, 0.16, 600, 5200);
  nz(t + 0.10, 0.10, 0.14, 600, 5200);
  [72, 76, 79, 84, 88].forEach((m, i) => tone(t + 0.24 + i * 0.08, m, 0.26, 0.14, 'triangle'));
  pad(t + 0.24, [60, 64, 67], 1.2, 0.24);
}
