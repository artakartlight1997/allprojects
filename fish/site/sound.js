...
const BGM = {
  prog: [[0, 5, 7, 5], [0, 3, 5, 3]],
  min: [[], [1, 3]],
};

let bgmOn = false, bgmRoot = 57, bgmSet = 0, bgmBpm = 86, bgmHot = 0;
...
function sfxCast() {
  if (!A.ctx) return;
  const t = anow();
  nz(t, 0.22, 0.10, 500, 4000);
  tone(t + 0.18, 60, 0.12, 0.09, 'sine', A.sfx, 48);
}
function sfxBite() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 84, 0.07, 0.14, 'square');
  tone(t + 0.09, 89, 0.09, 0.14, 'square');
}
function sfxHook() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 55, 0.18, 0.14, 'sawtooth', A.sfx, 67);
  nz(t, 0.18, 0.10, 300, 2600);
}
function sfxNushi() {
  if (!A.ctx) return;
  const t = anow();
  [48, 55, 60, 67, 72, 79, 84].forEach((m, i) => tone(t + i * 0.09, m, 0.34, 0.15, 'triangle'));
  pad(t, [36, 43, 48], 1.8, 0.30);
  kick(t, 0.9); kick(t + 0.36, 0.9);
}
function sfxJunk() {
  if (!A.ctx) return;
  const t = anow();
  nz(t, 0.14, 0.12, 200, 1400);
  tone(t + 0.10, 50, 0.22, 0.10, 'square', A.sfx, 43);
}
