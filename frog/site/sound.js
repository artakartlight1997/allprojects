...
const BGM = { prog: [[0, 5, 7, 5], [0, 3, 5, 7]], min: [[], [1]] };

let bgmOn = false, bgmRoot = 60, bgmSet = 0, bgmBpm = 126, bgmHot = 0;
...
function sfxHop() {
  if (!A.ctx) return;
  tone(anow(), 67, 0.06, 0.11, 'square', A.sfx, 76);
}
function sfxHome() {
  if (!A.ctx) return;
  const t = anow();
  bleep(t, [72, 76, 79, 84, 88], 0.05, 0.11, 0.14);
  kick(t, 0.6);
}
function sfxDead(kind) {
  if (!A.ctx) return;
  const t = anow();
  if (kind === 'water') {
    nz(t, 0.35, 0.14, 200, 2600);
    bleep(t, [60, 55, 50], 0.10, 0.16, 0.11);
  } else if (kind === 'time') {
    bleep(t, [72, 67, 62, 57], 0.10, 0.16, 0.13);
  } else {
    nz(t, 0.18, 0.18, 100, 2000);
    bleep(t + 0.05, [55, 50, 45], 0.09, 0.16, 0.13);
  }
}
