...
//   数字も じかんの ぼうも ぜんぶ 上の ほそい おび（HUD_H）に まとめ、
...
function drawHud(B) {
  ...
  retroText('おうち', 204, 5, 12, PAL.gy, null);
  drawNum(G.homes.filter((h) => h).length + '/' + G.HOMES.length, 246, 6, 2, PAL.g, 'left');

  // のこり じかんの ぼう（上の おびの 中）
  const bw = Math.min(160, VW * 0.18), bx = 352, by = 7;
  ctx.fillStyle = 'rgba(248,248,248,0.18)';
  ctx.fillRect(bx, by, bw, 8);
  const k = Math.max(0, G.left / G.S.sec);
  ctx.fillStyle = k > 0.4 ? PAL.g : k > 0.2 ? PAL.y : PAL.r;
  ctx.fillRect(bx, by, bw * k, 8);
  retroText('じかん', bx - 44, 5, 12, PAL.gy, null);

  // のこりの カエルと めんの 名まえ
  let lx = bx + bw + 12;
  for (let i = 0; i < G.lives - 1; i++) drawFrog(lx + i * 15, 3, 14, 0, false);
  lx += Math.max(0, G.lives - 1) * 15 + 6;
  retroText(G.S.name, lx, 5, 12, PAL.c, null);

  // ★ おしらせは ばんめんの 上に かさねると おうちが 見えなく なるので、
  //   いちばん 下の あいた ところに 出す。
  if (G.msgT > 0 && G.msg) {
    ctx.globalAlpha = Math.min(1, G.msgT * 2);
    retroText(G.msg, VW / 2, VH * 0.9, 15, PAL.y, PAL.dk, 'center');
    ctx.globalAlpha = 1;
  }
}
...
function drawHowto() {
  ...
  const lines = [
    '① 画面の どこでも ゆびを おいて すべらせると 1ますずつ はねる',
