...
function sfxFlap() {
  if (!A.ctx) return;
  nz(anow(), 0.10, 0.16, 300, 1400, A.sfx);
  tone(anow(), 76, 0.14, 0.13, 'sine', A.sfx, 84);
}
function sfxShoot() { if (A.ctx) tone(anow(), 84, 0.10, 0.10, 'sine', A.sfx, 90); }
function sfxPop() {
  if (!A.ctx) return;
  nz(anow(), 0.06, 0.30, 900, 4500, A.sfx);
  tone(anow(), 88, 0.12, 0.16, 'triangle', A.sfx, 96);
}
function sfxHurt() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 64, 0.28, 0.26, 'square', A.sfx, 48);
  nz(t, 0.16, 0.22, 200, 1200, A.sfx);
}
function sfxItem() {
  if (!A.ctx) return;
  const t = anow();
  [0, 4, 7, 12].forEach((d, i) => tone(t + i * 0.06, 79 + d, 0.20, 0.20, 'triangle', A.sfx));
}
function sfxBoss() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 40, 0.7, 0.30, 'sawtooth', A.sfx, 34);
  nz(t, 0.5, 0.25, 60, 500, A.sfx);
}
function sfxClear(win) {
  if (!A.ctx) return;
  const t = anow() + 0.05;
  const up = win ? [0, 4, 7, 12, 16, 19] : [0, -2, -5];
  const root = win ? 72 : 62;
  up.forEach((d, i) => tone(t + i * 0.13, root + d, 0.55, 0.26, 'triangle', A.sfx));
}
function sfxTest() {
  audioStart();
  if (!A.ctx) return;
