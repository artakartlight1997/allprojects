...
function sfxPlace() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 60, 0.10, 0.13, 'square', A.sfx, 72);
  nz(t, 0.07, 0.09, 500, 2600);
}
function sfxUp() {
  if (!A.ctx) return;
  const t = anow();
  [72, 79, 84].forEach((m, i) => tone(t + i * 0.05, m, 0.16, 0.11, 'triangle'));
}
function sfxNo() {
  if (!A.ctx) return;
  tone(anow(), 54, 0.12, 0.10, 'square', A.sfx, 48);
}
// うつ 音。とうの しゅるいで かえる。
function sfxShot(k) {
  if (!A.ctx) return;
  const t = anow();
  if (k === 'punch') { nz(t, 0.05, 0.09, 700, 3000); tone(t, 67, 0.05, 0.07, 'square'); }
  else if (k === 'bomb') { nz(t, 0.09, 0.11, 200, 1400); }
  else if (k === 'ice') { tone(t, 91, 0.07, 0.05, 'sine', A.sfx, 84); }
  else { tone(t, 84, 0.09, 0.07, 'sawtooth', A.sfx, 96); }
}
function sfxBoom() {
  if (!A.ctx) return;
  const t = anow();
  nz(t, 0.26, 0.20, 120, 2200);
  tone(t, 46, 0.24, 0.11, 'square', A.sfx, 34);
}
// てきが たおれた
function sfxDie() {
  if (!A.ctx) return;
  tone(anow(), 62, 0.11, 0.07, 'triangle', A.sfx, 50);
}
// てきが きちに 入った
function sfxLeak() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 55, 0.30, 0.15, 'square', A.sfx, 40);
  nz(t, 0.18, 0.10, 150, 900);
}
// つぎの なみが 来る
function sfxWave(n) {
  if (!A.ctx) return;
  const t = anow();
  [67, 74].forEach((m, i) => tone(t + i * 0.10, m + Math.min(6, n), 0.18, 0.13, 'triangle'));
}
function sfxBoss() {
  if (!A.ctx) return;
  const t = anow();
  [48, 48, 51].forEach((m, i) => tone(t + i * 0.16, m, 0.30, 0.16, 'sawtooth'));
  nz(t, 0.5, 0.14, 100, 1200);
}
function sfxEnd(win) {
  if (!A.ctx) return;
  const t = anow();
  const ms = win ? [72, 76, 79, 84, 88] : [72, 68, 63, 58];
...
function sfxTest() {
  ...
  kick(t, 0.75); kick(t + 0.3, 0.75);
}

// --- BGM ----------------------------------------------------------------------

const BGM = {
  prog: [[0, 3, 5, 7], [0, 5, 3, 8], [0, 7, 5, 3]],
  min: [[0, 1, 2, 3], [0, 2], [0, 3]],
};
...
function bgmPump() {
  ...
  const spb = 60 / (bgmBpm * (1 + bgmHot * 0.08));
  const barLen = spb * 4;
...
function bgmBar(t0, bar, spb) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (i === 1 || i === 3) nz(t0 + i * spb, 0.08, 0.16, 1000, 4200, A.mus);
  }
  for (let i = 0; i < 8; i++) {
    tone(t0 + i * (spb / 2), r - 24 + (i % 4 === 3 ? 5 : 0), spb * 0.42, 0.16, 'square', A.mus);
  }
  pad(t0, [r, r + third, r + 7], spb * 3.8, 0.12);
  if (bgmHot > 0.5) {
    const arp = [12, 7, third, 7];
    for (let i = 0; i < 4; i++) tone(t0 + i * spb, r + arp[i], spb * 0.8, 0.06, 'sawtooth', A.mus);
  }
}
