...
function mapBox() {
  ...
  const top = 48, bot = VH - 12;
  const h = bot - top;
  const s = h / (B.y1 - B.y0);
  const w = (B.x1 - B.x0) * s;
  return { s, ox: 34 - B.x0 * s, oy: top - B.y0 * s, w, h, x: 34, y: top, B };
}
...
function drawPlay(t) {
  ...
  for (const p of G.list) {
    ...
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    const showName = G.S.mode !== 'name' || G.askDone.indexOf(p.key) >= 0;
    if (showName) {
      ctx.font = 'bold 14px system-ui, sans-serif';
      ctx.fillText(p.name, s.x + dx, s.y);
    }
    ctx.textAlign = 'left';
