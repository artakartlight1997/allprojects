...
const BGM = { prog: [[0, 5, 3, 7], [0, 3, 7, 5]], min: [[1], [2]] };

let bgmOn = false, bgmRoot = 57, bgmSet = 0, bgmBpm = 132, bgmHot = 0;
...
// おかしを 食べる 音。かわりばんこで 高さを 変えると「パクパク」に 聞こえる
let dotHi = false;
function sfxDot() {
  if (!A.ctx) return;
  dotHi = !dotHi;
  tone(anow(), dotHi ? 72 : 67, 0.035, 0.09, 'square');
}
function sfxPower() {
  if (!A.ctx) return;
  const t = anow();
  bleep(t, [60, 64, 67, 72, 76, 79, 84], 0.04, 0.09, 0.13);
}
// つづけて 食べるほど 高く なる
function sfxEatGhost(n) {
  if (!A.ctx) return;
  const t = anow();
  const b = 64 + Math.min(4, n) * 4;
  bleep(t, [b, b + 5, b + 9, b + 12], 0.05, 0.10, 0.14);
}
function sfxDead() {
  if (!A.ctx) return;
  const t = anow();
  bleep(t, [72, 69, 65, 62, 58, 55, 51, 48, 44], 0.09, 0.16, 0.14);
  nz(t + 0.8, 0.30, 0.09, 120, 900);
}
