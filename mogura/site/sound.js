...
function sfxPop() {
  if (!A.ctx) return;
  tone(anow(), 64, 0.07, 0.07, 'triangle', A.sfx, 72);
}
// たたいた
function sfxHit() {
  if (!A.ctx) return;
  const t = anow();
  nz(t, 0.07, 0.16, 300, 2400);
  tone(t, 72, 0.09, 0.11, 'square', A.sfx, 56);
}
// たたいちゃ だめな もの
function sfxBad() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 58, 0.24, 0.14, 'square', A.sfx, 44);
  nz(t, 0.14, 0.10, 150, 1000);
}
// にがした
function sfxLet() {
  if (!A.ctx) return;
  tone(anow(), 60, 0.12, 0.06, 'sine', A.sfx, 52);
}
// れんぞくで たたけた
function sfxCombo(n) {
  if (!A.ctx) return;
  tone(anow(), 72 + Math.min(12, n) * 2, 0.10, 0.08, 'triangle');
}
// ボス（パパ）とうじょう
function sfxPapa() {
  if (!A.ctx) return;
  const t = anow();
  [48, 52, 55, 60].forEach((m, i) => tone(t + i * 0.10, m, 0.26, 0.14, 'sawtooth'));
  nz(t, 0.4, 0.12, 90, 900);
}
// パパを たおした
function sfxPapaDown() {
  if (!A.ctx) return;
  const t = anow();
  [72, 67, 60, 55, 48].forEach((m, i) => tone(t + i * 0.09, m, 0.22, 0.13, 'square'));
  nz(t, 0.3, 0.14, 120, 1600);
}
function sfxEnd(win) {
  if (!A.ctx) return;
  const t = anow();
...
const BGM = {
  prog: [[0, 5, 3, 7], [0, 7, 5, 3], [0, 3, 5, 7]],
  min: [[2], [3], [1]],
};

let bgmOn = false, bgmRoot = 57, bgmSet = 0, bgmBpm = 124, bgmHot = 0;
...
function bgmBar(t0, bar, spb) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (i === 1 || i === 3) nz(t0 + i * spb, 0.08, 0.16, 1000, 4400, A.mus);
    nz(t0 + i * spb + spb / 2, 0.03, 0.05, 6000, 11000, A.mus);
  }
  for (let i = 0; i < 8; i++) {
    tone(t0 + i * (spb / 2), r - 24 + (i % 4 === 2 ? 7 : 0), spb * 0.44, 0.15, 'square', A.mus);
  }
  pad(t0, [r, r + third, r + 7], spb * 3.8, 0.12);
  const mel = [12, 7, third, 12, 7, third];
  for (let i = 0; i < 6; i++) {
    if (bgmHot < 0.5 && i % 2 === 1) continue;
    tone(t0 + i * (spb * 4 / 6), r + mel[i], spb * 0.5, 0.05, 'triangle', A.mus);
  }
}
