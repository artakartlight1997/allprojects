...
初見殺しの仕掛けはステージ4以降にだけ置く。1〜3 は操作を覚える面なので、
知らないと避けられない要素は入れない。
...
def s1_grass():
    """基本操作。踏みつけとコイン。"""
    g = Grid(100)
    # 穴は 2 タイル。3 タイルだと踏み切りが少しずれただけで落ちる。
    for a, b in [(0, 27), (30, 54), (57, 71), (74, 99)]:
        g.ground(a, b, 10)
...
def s2_meadow():
    """ジャンプ台とダッシュシューズの導入。"""
    g = Grid(105)
    # 穴は 2 タイル。ジャンプ台に慣れる面なので、足場は易しくしておく。
    for a, b in [(0, 25), (28, 51), (54, 75), (78, 104)]:
        g.ground(a, b, 10)
    for x in (22, 46, 70):
        g.bounce(x, 9)
...
def s9_night():
    ...
    もとは「いちばん難しい」面だったが、子どもがクリアできなかったので
    やさしくした。空中の足場を 3〜4 タイルから 5〜6 タイルに広げ、
    すき間は 2 タイルまでにしてある（ジャンプは 5.96 タイル跳べる）。
    初見殺しの仕掛けと、追いかけてくる敵はこの面には置いていない。
    移動床は足場の真上に置いてあるので、乗りそこねても下の足場に落ちる。
    """
    g = Grid(130)
    g.ground(0, 17, 10)
    g.ground(55, 73, 10)
    ...
    # 空の区間。高さの差は 1 タイルずつなので、順に跳んでいける
    plats = [(19, 24, 8), (27, 32, 7), (35, 40, 6), (43, 48, 7), (51, 54, 8),
             (75, 80, 8), (83, 88, 7), (91, 96, 6), (99, 104, 7), (107, 111, 8)]
    for a, b, y in plats:
        g.plat(a, b, y)

    # 移動床は足場の真上。使わなくても先へ進めるので、ごほうび用
    g.mover(21, 5, vertical=True)
    g.mover(45, 4)
    g.mover(77, 5, vertical=True)
    g.mover(101, 4)
    ...
    for a, b, y in plats:
        g.coins(a, b, y - 1)
    for a, b in [(24, 27), (32, 35), (40, 43), (48, 51),
                 (80, 83), (88, 91), (96, 99), (104, 107)]:
        g.arc(a, b, 5, 2)
    g.coins(122, 127, 9)

    # 敵は 7 体。追いかけてくる「おいかけ」は置かない
    g.put(14, 9, 'w')
    g.put(30, 6, 'w')
    g.put(65, 9, 'j')
    g.put(93, 5, 'k')
    for x, y in [(16, 4), (59, 3), (105, 4)]:
        g.put(x, y, 'p')

    # はねは空の区間に入る手前に置く。2 段ジャンプで足場をはずしても届く
    g.put(12, 9, 'f')
    g.put(70, 9, 'f')
    g.put(53, 7, 'b')
    g.put(58, 9, 'h')
    ...
    # 中間地点は多めに。落ちてもすぐ近くから再開できる
    for x, y in [(16, 9), (46, 6), (57, 9), (72, 9), (94, 5)]:
        g.put(x, y, 'C')
