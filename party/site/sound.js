...
function sfxCount(n) {
  if (!A.ctx) return;
  tone(anow(), 69 + (n === 0 ? 7 : 0), 0.16, 0.22, 'square', A.sfx);
}
// 「いま！」の あいず。いちばん 大事な 音なので 目立たせる。
function sfxGo() {
  if (!A.ctx) return;
  const t = anow();
  [0, 7, 12].forEach((d, i) => tone(t + i * 0.03, 81 + d, 0.28, 0.26, 'square', A.sfx));
  nz(t, 0.05, 0.2, 2000, 7000, A.sfx);
}
// ボタンを おした
function sfxTap(p) {
  if (!A.ctx) return;
  tone(anow(), 74 + p * 3, 0.07, 0.16, 'triangle', A.sfx, 84 + p * 3);
}
// せいかい／まちがい
function sfxGood() {
  if (!A.ctx) return;
  const t = anow();
  [0, 4, 7].forEach((d, i) => tone(t + i * 0.05, 79 + d, 0.22, 0.20, 'triangle', A.sfx));
}
function sfxBad() {
  if (!A.ctx) return;
  nz(anow(), 0.16, 0.26, 150, 1200, A.sfx);
  tone(anow(), 55, 0.22, 0.18, 'sawtooth', A.sfx, 45);
}
// ラウンドの おわり
function sfxRound() {
  if (!A.ctx) return;
  const t = anow();
  [0, 5, 9, 12].forEach((d, i) => tone(t + i * 0.09, 74 + d, 0.34, 0.22, 'triangle', A.sfx));
  kick(t, 0.6);
}
// ゆうしょう
function sfxWin() {
...
const BGM = {
  prog: [[0, 5, 7, 5], [0, 9, 5, 7], [0, 7, 9, 5]],
  min: [[], [1], [2]],
};

let bgmOn = false, bgmRoot = 62, bgmSet = 0, bgmBpm = 128;

function bgmStart(i) {
  audioStart();
  if (!A.ctx) return;
  bgmOn = true;
  bgmSet = i % BGM.prog.length;
  bgmRoot = 60 + (i % 3) * 2;
  bgmBpm = 124 + (i % 4) * 6;
  A.bgmT = anow() + 0.15;
  A.bgmBar = 0;
...
function bgmBar(t0, bar, spb) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    nz(t0 + i * spb + spb / 2, 0.03, 0.06, 6000, 11000, A.mus);
  }
