...
const BGM = {
  prog: [[0, 7, 5, 3], [0, 5, 7, 10]],
  min: [[3], [2]],
};

let bgmOn = false, bgmRoot = 55, bgmSet = 0, bgmBpm = 134, bgmHot = 0;
...
function sfxLaunch() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 45, 0.24, 0.16, 'sawtooth', A.sfx, 70);
  nz(t, 0.18, 0.10, 400, 3000);
}
// 当てるほど 高く なる（つながって いるのが 耳で わかる）
function sfxBump(n) {
  if (!A.ctx) return;
  tone(anow(), 72 + n * 2, 0.09, 0.14, 'square', A.sfx, 79 + n * 2);
}
function sfxTarget() {
  if (!A.ctx) return;
  const t = anow();
  tone(t, 84, 0.07, 0.13, 'triangle', A.sfx, 91);
  nz(t, 0.05, 0.08, 2000, 8000);
}
function sfxBonus() {
  if (!A.ctx) return;
  const t = anow();
  [72, 76, 79, 84, 88, 91, 96].forEach((m, i) => tone(t + i * 0.06, m, 0.24, 0.14, 'triangle'));
  pad(t, [48, 55, 60, 64], 1.4, 0.26);
}
function sfxFlip() {
  if (!A.ctx) return;
  const t = anow();
  nz(t, 0.05, 0.12, 600, 4000);
  tone(t, 55, 0.06, 0.09, 'square');
}
function sfxDrain() {
  if (!A.ctx) return;
  const t = anow();
  [60, 55, 50, 44].forEach((m, i) => tone(t + i * 0.09, m, 0.26, 0.12, 'triangle'));
}
