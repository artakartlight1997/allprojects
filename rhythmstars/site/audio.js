...
// こえ みたいな 音。「わっ」「ホイ」など。
// 口の 形（フォルマント）を バンドパスの フィルタ 2つで まねている。
function voice(t, m, dur, v, dst) {
  if (!A.ctx) return;
  t = safeT(t);
  const o = A.ctx.createOscillator(), g = A.ctx.createGain();
  o.type = 'sawtooth';
  o.frequency.setValueAtTime(mid2f(m) * 0.98, t);
  o.frequency.linearRampToValueAtTime(mid2f(m), t + 0.05);
  ...
  const g2 = A.ctx.createGain(); g2.gain.value = 0.6;
  g.gain.setValueAtTime(0.0001, t);
  g.gain.exponentialRampToValueAtTime(v, t + 0.03);
  g.gain.setValueAtTime(v, t + dur * 0.6);
  g.gain.exponentialRampToValueAtTime(0.0001, t + dur);
  o.connect(f1); o.connect(f2);
  f1.connect(g); f2.connect(g2); g2.connect(g);
  g.connect(dst || A.sfx);
  o.start(t); o.stop(t + dur + 0.05);
}
...
function sfxHit(kind, good) {
  ...
  if (kind === 'punch') {
    ...
    nzHit(t + 0.02, 0.05, 0.26, 1400, 6000, A.sfx);
    pluck(t + 0.04, 93, 0.14, 0.18, A.sfx);
  } else if (kind === 'stomp') {
    // タップシューズの カツン
    nzHit(t, 0.035, 0.5, 2600, 9000, A.sfx);
    tom(t, 260, 0.34, A.sfx);
  } else if (kind === 'ribbit') {
    // かえるの こえ
    voice(t, 55, 0.18, 0.26, A.sfx);
    glide(t, 220, 160, 0.16, 0.14, 'square', 600, 3);
  } else if (kind === 'stamp') {
    // こうじょうの ハンマー
    kick(t, 0.9, A.sfx);
    nzHit(t, 0.07, 0.4, 900, 4500, A.sfx);
    pluck(t + 0.01, 64, 0.2, 0.2, A.sfx);
  } else if (kind === 'ghost') {
    // おばけを つかまえる
    glide(t, 900, 1800, 0.09, 0.24, 'triangle');
    nzHit(t, 0.06, 0.3, 1600, 7000, A.sfx);
    pluck(t + 0.03, 91, 0.2, 0.16, A.sfx);
  } else {
...
const DRUM = {
  ...
  wa:    { k: 'x...x.x.x...x...', s: '..s...s...s...s.', h: '....h.......h...' },
  swing: { k: 'x.....x.x.....x.', s: '....x.......x...', h: 'h..h..h..h..h..h' },
  night: { k: 'x.......x.......', s: '............s...', h: '....h...h...h...' },
};
