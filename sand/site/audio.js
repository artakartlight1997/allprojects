...
// --- 料理の おと ---------------------------------------------------------------
//
// 「たたいた かんじ」が 食べものと あって いないと 気もちよくない ので、
// ミニゲームの しゅるいごとに 音を つくる。

function sfxCook(kind, good) {
  if (!A.ctx) return;
  const t = anow() + 0.001;
  const v = good ? 1 : 0.72;
  if (kind === 'cut') {                 // トン！ と きる
    nzHit(t, 0.055, 0.46 * v, 1800, 7000, A.sfx);
    tom(t, 190, 0.34 * v, A.sfx);
  } else if (kind === 'grill') {        // ジュッ！ と やく
    nzHit(t, 0.20, 0.34 * v, 700, 5200, A.sfx);
    pluck(t, 70, 0.16, 0.16 * v, A.sfx);
  } else if (kind === 'spread') {       // シャッ と ぬる
    nzHit(t, 0.13, 0.28 * v, 1400, 4200, A.sfx);
  } else if (kind === 'place') {        // ポンッ と のせる
    pluck(t, 86, 0.14, 0.30 * v, A.sfx);
    nzHit(t, 0.04, 0.24 * v, 900, 3200, A.sfx);
  } else if (kind === 'mix') {          // クルクル まぜる
    pluck(t, 79, 0.18, 0.24 * v, A.sfx);
    pluck(t + 0.045, 86, 0.16, 0.18 * v, A.sfx);
  } else if (kind === 'shake') {        // サッ と ふる
    nzHit(t, 0.07, 0.30 * v, 3500, 9000, A.sfx);
  } else if (kind === 'squeeze') {      // ニュルッ と しぼる
    const o = A.ctx.createOscillator(), g = A.ctx.createGain();
    o.type = 'sawtooth';
  ...
  } else if (kind === 'wrap') {         // クシャッ と つつむ
    nzHit(t, 0.16, 0.26 * v, 2200, 8000, A.sfx);
    nzHit(t + 0.07, 0.10, 0.16 * v, 1600, 6000, A.sfx);
  } else {
    nzHit(t, 0.06, 0.4 * v, 1000, 4000, A.sfx);
    pluck(t, 84, 0.14, 0.26 * v, A.sfx);
  }
  if (good) pluck(t + 0.01, 96, 0.10, 0.10, A.sfx);
}
...
function sfxDone() {
  if (!A.ctx) return;
  const t = anow() + 0.03;
  [0, 4, 7, 12, 16, 19].forEach((d, i) => pluck(t + i * 0.075, 72 + d, 0.5, 0.28, A.sfx));
  kick(t, 0.7); kick(t + 0.3, 0.6);
}
...
function sfxBite(n) {
  if (!A.ctx) return;
  const t = anow() + 0.001;
  nzHit(t, 0.13, 0.42, 600, 4800, A.sfx);
  tom(t, 140 - n * 8, 0.4, A.sfx);
}
