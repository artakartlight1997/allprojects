...
function startStage(i) {
  ...
  if (m === 'fit' || m === 'fit2' || m === 'time') {
    ...
    // learn / name / item / where … ぜんぶ はめた ところから 出す
...
// ★ 地図を まわす。画面の どこに あるかを おぼえた だけの 子は
//   まわると わからなく なる。「北がわ」「西がわ」で おぼえて はじめて とける。
function spinMap() {
  let n;
  do { n = (Math.random() * 4) | 0; } while (n * (Math.PI / 2) === G.spin && Math.random() < 0.8);
  G.spin = n * (Math.PI / 2);
}

// いま きかれて いる ことば（めんの しゅるいで かわる）
function askText() {
  if (!G.ask) return '';
  const m = G.S.mode;
  if (m === 'item') return G.ask.item + ' を とどけて！';
  if (m === 'where') return G.ask.where + 'の けんは どこ？';
  return G.ask.name + 'けん は どこ？';
}
