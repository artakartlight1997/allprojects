...
function drawPlay(t) {
  ...
  //   HUD の はしらと かさならない ように、たなの となり（右上）に 置く。
  {
    const cx = (M.tray ? M.tray.x : VW - PAD) - 34;
    const cy = M.hud === 'top' ? 74 : 40;
...
function ASK_TEXT() { return askText(); }
...
function move(px, py) {
  ...
  G.hold.x = px / SC; G.hold.y = py / SC - VOY;
}
function up() {
  if (!dragging) return;
  dragging = false;
